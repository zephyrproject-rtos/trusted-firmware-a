...
#include <drivers/renesas/rza/cpg/cpg_regs.h>
#include <drivers/renesas/rza/pfc/pfc_regs.h>
...
static int spim_exec_op(xspi_ctrl_t *const ctrl, xspi_op_t const *const op,
			bool is_write);
static int spim_configure_xip(xspi_ctrl_t *const ctrl,
			      xspi_op_t const *const rop,
			      xspi_op_t const *const wop);
...
/* Static variables */
static bool globally_initialised;
...
static void select_spim(spim_ctrl_t *myctrl, xspi_cfg_t const *const cfg)
{
	if (RZ_XSPI_EXCLUSIVE_SELECTOR) {
		...
		if ((ipcont_spi_octa & IPCONT_SEL_SPI_OCTA) ==
		    IPCONT_SEL_SPI_OCTA_OCTA) {
			 ...
			 */
			/* Assert reset line */
			mmio_clrbits_32(myctrl->reg_base + OCTA_RSTCNT,
					OCTA_RSTCNT_RSTVAL);
...
{
	wait_until_32(myctrl->reg_base + SPIM_CMNSR, SPIM_CMNSR_TEND,
		      SPIM_CMNSR_TEND);
}
...
static const uint32_t cmncr_init_mask = SPIM_CMNCR_BSZ | SPIM_CMNCR_IO0FV |
					SPIM_CMNCR_IO2FV | SPIM_CMNCR_IO3FV |
					SPIM_CMNCR_MOIIO0 | SPIM_CMNCR_MOIIO1 |
					SPIM_CMNCR_MOIIO2 | SPIM_CMNCR_MOIIO3;
...
static const uint32_t cmncr_init_value =
	1u << SPIM_CMNCR_MD_POS | SPIM_CMNCR_BSZ_SINGLE << SPIM_CMNCR_BSZ_POS |
...
static const uint32_t ssldr_init_mask = SPIM_SSLDR_SCKDL | SPIM_SSLDR_SLNDL |
					SPIM_SSLDR_SPNDL;
static const uint32_t ssldr_init_value = 0u << SPIM_SSLDR_SCKDL_POS |
					 0u << SPIM_SSLDR_SLNDL_POS |
					 0u << SPIM_SSLDR_SPNDL_POS;

static const uint32_t drcr_init_mask = SPIM_DRCR_RBURST | SPIM_DRCR_RCF |
				       SPIM_DRCR_RBE | SPIM_DRCR_SSLE;
static const uint32_t drcr_init_value =
	7u << SPIM_DRCR_RBURST_POS | 1u << SPIM_DRCR_RCF_POS |
	1u << SPIM_DRCR_RBE_POS | 1u << SPIM_DRCR_SSLE_POS;

static const uint32_t drear_init_mask = SPIM_DREAR_EAC | SPIM_DREAR_EAV;
static const uint32_t drear_init_value = 3u << SPIM_DREAR_EAC_POS |
					 0u << SPIM_DREAR_EAV_POS;

static const uint32_t drdrenr_init_mask =
	SPIM_DRDRENR_HYPE | SPIM_DRDRENR_ADDRE | SPIM_DRDRENR_OPDRE |
	SPIM_DRDRENR_DRDRE;
static const uint32_t drdrenr_init_value =
	0u << SPIM_DRDRENR_HYPE_POS | 0u << SPIM_DRDRENR_ADDRE_POS |
	0u << SPIM_DRDRENR_OPDRE_POS | 0u << SPIM_DRDRENR_DRDRE_POS;

static const uint32_t phycnt_init_mask =
	SPIM_PHYCNT_ALT_ALIGN | SPIM_PHYCNT_CAL | SPIM_PHYCNT_CKSEL |
	SPIM_PHYCNT_EXDS | SPIM_PHYCNT_HS | SPIM_PHYCNT_OCT | SPIM_PHYCNT_OCTA |
	SPIM_PHYCNT_PHYMEM | SPIM_PHYCNT_WBUF2 | SPIM_PHYCNT_WBUF;
static const uint32_t phycnt_init_value =
	0u << SPIM_PHYCNT_ALT_ALIGN_POS | 0u << SPIM_PHYCNT_CAL_POS |
	3u << SPIM_PHYCNT_CKSEL_POS | 0u << SPIM_PHYCNT_EXDS_POS |
	0u << SPIM_PHYCNT_HS_POS | 0u << SPIM_PHYCNT_OCT_POS |
	0u << SPIM_PHYCNT_OCTA_POS | 0u << SPIM_PHYCNT_PHYMEM_POS |
	0u << SPIM_PHYCNT_WBUF2_POS | 0u << SPIM_PHYCNT_WBUF_POS;
...
static const uint32_t phyoffset1_init_value = SPIM_PHYOFFSET1_SDR
					      << SPIM_PHYOFFSET1_DDRTMG_POS;

static const uint32_t phyoffset2_init_mask = SPIM_PHYOFFSET2_OCTTMG;
static const uint32_t phyoffset2_init_value = SPIM_PHYOFFSET2_SPI
					      << SPIM_PHYOFFSET2_OCTTMG_POS;

static void spim_ip_init(spim_ctrl_t *myctrl)
{
	cpg_set_xspi_clock(XSPI_CLOCK_SPIM, spi_clock * 4);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_CMNCR, cmncr_init_mask,
			   cmncr_init_value);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_SSLDR, ssldr_init_mask,
			   ssldr_init_value);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_DRCR, drcr_init_mask,
			   drcr_init_value);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_DREAR, drear_init_mask,
			   drear_init_value);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_DRDRENR, drdrenr_init_mask,
			   drdrenr_init_value);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYCNT, phycnt_init_mask,
			   phycnt_init_value | SPIM_PHYCNT_CAL);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYOFFSET1,
			   phyoffset1_init_mask, phyoffset1_init_value);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYOFFSET2,
			   phyoffset2_init_mask, phyoffset2_init_value);
...
static uint32_t spim_cmncr_set(uint8_t mask, uint8_t value, int pos)
{
	int moiionpos = -1;
	int ionfvpos = -1;
	...
	if (mask & (1 << pos)) {
		moiio = value & 1 << pos ? SPIM_CMNCR_IO_HIGH :
					   SPIM_CMNCR_IO_LOW;
...
static void spim_set_idlelevel(spim_ctrl_t *const myctrl,
			       const xspi_op_t *const op)
{
	static const uint32_t cmncr_mask =
		SPIM_CMNCR_IO2FV | SPIM_CMNCR_IO3FV | SPIM_CMNCR_MOIIO1 |
		SPIM_CMNCR_MOIIO2 | SPIM_CMNCR_MOIIO3;

	uint32_t cmncr_set = 0;

	cmncr_set |= spim_cmncr_set(op->force_idle_level_mask,
				    op->force_idle_level_value, 1);
	cmncr_set |= spim_cmncr_set(op->force_idle_level_mask,
				    op->force_idle_level_value, 2);
	cmncr_set |= spim_cmncr_set(op->force_idle_level_mask,
				    op->force_idle_level_value, 3);

	mmio_clrsetbits_32(myctrl->reg_base + SPIM_CMNCR, cmncr_mask,
			   cmncr_set);
}
...
static int spim_stop_xip_internal(spim_ctrl_t *myctrl)
{
	...
	if ((drcr & (SPIM_DRCR_RBE | SPIM_DRCR_SSLE)) ==
	    (SPIM_DRCR_RBE | SPIM_DRCR_SSLE)) {
		/* Set SSLN and wait for sslf */
		mmio_write_32(myctrl->reg_base + SPIM_DRCR,
			      drcr | SPIM_DRCR_SSLN);
		test_sslf(myctrl);
	}
	test_tend(myctrl);
...
static bool spim_stop_xip_temporarily(spim_ctrl_t *myctrl)
{
	...
	bool state =
		!(mmio_read_32(myctrl->reg_base + SPIM_CMNCR) & SPIM_CMNCR_MD);
	spim_stop_xip_internal(myctrl);
	return state;
}

static const uint32_t smcmr_clearmask = SPIM_SMCMR_CMD | SPIM_SMCMR_OCMD;

static const uint32_t smenr_clearmask =
	SPIM_SMENR_ADB | SPIM_SMENR_ADE | SPIM_SMENR_CDB | SPIM_SMENR_CDE |
	SPIM_SMENR_DME | SPIM_SMENR_SPIDB | SPIM_SMENR_OCDB | SPIM_SMENR_OCDE |
	SPIM_SMENR_OPDB | SPIM_SMENR_OPDE | SPIM_SMENR_SPIDE;

static const uint32_t smdrenr_clearmask =
	SPIM_SMDRENR_ADDRE | SPIM_SMDRENR_DRDRE | SPIM_SMDRENR_HYPE |
	SPIM_SMDRENR_OPDRE;

static const uint32_t smdmcr_clearmask = SPIM_SMDMCR_DMCYC;

static const uint32_t ssldr_clearmask = SPIM_SSLDR_SCKDL | SPIM_SSLDR_SLNDL |
					SPIM_SSLDR_SPNDL;
...
static const uint32_t smenr_op_none = 0u << SPIM_SMENR_CDE_POS |
				      0u << SPIM_SMENR_OCDE_POS;

/* SMENR value for 1-byte opcode */
static const uint32_t smenr_op_1byte = 1u << SPIM_SMENR_CDE_POS |
				       0u << SPIM_SMENR_OCDE_POS;

/* SMENR value for 2-byte opcode */
static const uint32_t smenr_op_2byte = 1u << SPIM_SMENR_CDE_POS |
				       1u << SPIM_SMENR_OCDE_POS;

/* SMENR value for no address */
static const uint32_t smenr_addr_none = SPIM_SMENR_ADE_NONE
					<< SPIM_SMENR_ADE_POS;

/* SMENR value for 3-byte address */
static const uint32_t smenr_addr_3byte = SPIM_SMENR_ADE_3BYTE
					 << SPIM_SMENR_ADE_POS;

/* SMENR value for 4-byte address */
static const uint32_t smenr_addr_4byte = SPIM_SMENR_ADE_4BYTE
					 << SPIM_SMENR_ADE_POS;

/* SMENR value for no additional data */
static const uint32_t smenr_additional_none = SPIM_SMENR_OPDE_NONE
					      << SPIM_SMENR_OPDE_POS;

/* SMENR value for 1-byte additional data */
static const uint32_t smenr_additional_1byte = SPIM_SMENR_OPDE_1BYTE
					       << SPIM_SMENR_OPDE_POS;

/* SMENR value for 2-byte additional data */
static const uint32_t smenr_additional_2byte = SPIM_SMENR_OPDE_2BYTE
					       << SPIM_SMENR_OPDE_POS;

/* SMENR value for 3-byte additional data */
static const uint32_t smenr_additional_3byte = SPIM_SMENR_OPDE_3BYTE
					       << SPIM_SMENR_OPDE_POS;

/* SMENR value for 4-byte additional data */
static const uint32_t smenr_additional_4byte = SPIM_SMENR_OPDE_4BYTE
					       << SPIM_SMENR_OPDE_POS;

static void send_256(spim_ctrl_t *myctrl, xspi_transfer_form_t form,
		     uintptr_t buffer, uint32_t smenr)
{
	/* Use wbuffer for transfer */
	mmio_setbits_32(myctrl->reg_base + SPIM_DRCR, SPIM_DRCR_RCF);
	mmio_setbits_32(myctrl->reg_base + SPIM_PHYCNT,
			SPIM_PHYCNT_WBUF2 | SPIM_PHYCNT_WBUF | SPIM_PHYCNT_CAL);
	if (form == SPI_FORM_1_1_4 || form == SPI_FORM_1_4_4) {
		uint32_t phyoffset2_msk = SPIM_PHYOFFSET2_OCTTMG;
		uint32_t phyoffset2_set = SPIM_PHYOFFSET2_SPI_WBUF
					  << SPIM_PHYOFFSET2_OCTTMG_POS;
		mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYOFFSET2,
				   phyoffset2_msk, phyoffset2_set);
	}
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_SMENR, smenr_clearmask,
			   smenr | (SPIM_SMENR_SPIDE_LONG
				    << SPIM_SMENR_SPIDE_POS));
	/* Write data to buffer */
	for (int bytes = 0; bytes < 256; bytes += 8) {
		mmio_write_64(myctrl->reg_base + SPIM_BUFFER + bytes,
			...
			bool is_write)
{
	...
	assert(op->transfer_size == 0 ||
	       (op->transfer_size && op->transfer_buffer));
	spim_ctrl_t *myctrl = (spim_ctrl_t *)ctrl;
	bool is_xip;
	...
	uint32_t save_phyoffset1 =
		mmio_read_32(myctrl->reg_base + SPIM_PHYOFFSET1);
	uint32_t save_phyoffset2 =
		mmio_read_32(myctrl->reg_base + SPIM_PHYOFFSET2);
	...
	case 2:
		...
		smopr |= (op->additional_value & 0xff00) >>
			 8 << SPIM_SMOPR_OPD3_POS;
		break;
	case 3:
		smenr_set |= smenr_additional_3byte;
		smopr = (op->additional_value & 0xff) << SPIM_SMOPR_OPD1_POS;
		smopr |= (op->additional_value & 0xff00) >>
			 8 << SPIM_SMOPR_OPD2_POS;
		smopr |= (op->additional_value & 0xff0000) >>
			 16 << SPIM_SMOPR_OPD3_POS;
		break;
	case 4:
		smenr_set |= smenr_additional_4byte;
		smopr = (op->additional_value & 0xff) << SPIM_SMOPR_OPD0_POS;
		smopr |= (op->additional_value & 0xff00) >>
			 8 << SPIM_SMOPR_OPD1_POS;
		smopr |= (op->additional_value & 0xff0000) >>
			 16 << SPIM_SMOPR_OPD2_POS;
		smopr |= (op->additional_value & 0xff000000) >>
			 24 << SPIM_SMOPR_OPD3_POS;
		break;
	default:
	...
	/* Dummy cycle */
	if (op->dummy_cycles == 1 || op->dummy_cycles > 20) {
	...
	/* Write the register */
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_SMCMR, smcmr_clearmask,
			   smcmr_set);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_SMDRENR, smdrenr_clearmask,
			   smdrenr_set);
	mmio_write_32(myctrl->reg_base + SPIM_SMADR, smadr);
	mmio_write_32(myctrl->reg_base + SPIM_SMOPR, smopr);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_SMDMCR, smdmcr_clearmask,
			   smdmcr_set);
	mmio_write_32(myctrl->reg_base + SPIM_SSLDR,
		      (save_ssldr & ~ssldr_clearmask) | ssldr_set);
	mmio_write_32(myctrl->reg_base + SPIM_PHYOFFSET1,
		      (save_phyoffset1 & ~phyoffset1_msk) | phyoffset1_set);
	mmio_write_32(myctrl->reg_base + SPIM_PHYCNT,
		      (save_phycnt & ~phycnt_msk) | phycnt_set |
			      SPIM_PHYCNT_CAL);
	...
	do {
		...
		if (!is_write && remain) {
			uint32_t smrdr =
				mmio_read_32(myctrl->reg_base + SPIM_SMRDR0);
			if (remain > 3) {
		...
		mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYCNT,
				   SPIM_PHYCNT_WBUF2 | SPIM_PHYCNT_WBUF,
				   SPIM_PHYCNT_CAL);
		mmio_write_32(myctrl->reg_base + SPIM_PHYOFFSET2,
			      save_phyoffset2);
		...
		if (remain > 0) {
			if (!is_write) {
				/* Increment address */
				smadr += xfer_count;
				mmio_write_32(myctrl->reg_base + SPIM_SMADR,
					      smadr);
			} else {
				/* Clear enabler bits for continuous write access */
				smenr_set &=
					~(SPIM_SMENR_DME | SPIM_SMENR_CDE |
					  SPIM_SMENR_OCDE | SPIM_SMENR_ADE |
					  SPIM_SMENR_OPDE);
			}
		}
	} while (remain > 0);
...
static const uint32_t drcmr_clearmask = SPIM_DRCMR_CMD | SPIM_DRCMR_OCMD;

static const uint32_t drenr_clearmask =
	SPIM_DRENR_ADB | SPIM_DRENR_ADE | SPIM_DRENR_CDB | SPIM_DRENR_CDE |
	SPIM_DRENR_DME | SPIM_DRENR_DRDB | SPIM_DRENR_OCDB | SPIM_DRENR_OCDE |
	SPIM_DRENR_OPDB | SPIM_DRENR_OPDE;

static const uint32_t drdrenr_clearmask =
	SPIM_DRDRENR_ADDRE | SPIM_DRDRENR_DRDRE | SPIM_DRDRENR_HYPE |
	SPIM_DRDRENR_OPDRE;

static const uint32_t drdmcr_clearmask = SPIM_DRDMCR_DMCYC;
...
static const uint32_t drenr_op_none = 0u << SPIM_DRENR_CDE_POS |
				      0u << SPIM_DRENR_OCDE_POS;

/* DRENR value for 1-byte opcode */
static const uint32_t drenr_op_1byte = 1u << SPIM_DRENR_CDE_POS |
				       0u << SPIM_DRENR_OCDE_POS;

/* DRENR value for 2-byte opcode */
static const uint32_t drenr_op_2byte = 1u << SPIM_DRENR_CDE_POS |
				       1u << SPIM_DRENR_OCDE_POS;

/* DRENR value for no address */
static const uint32_t drenr_addr_none = SPIM_DRENR_ADE_NONE
					<< SPIM_DRENR_ADE_POS;

/* DRENR value for 3-byte address */
static const uint32_t drenr_addr_3byte = SPIM_DRENR_ADE_3BYTE
					 << SPIM_DRENR_ADE_POS;

/* DRENR value for 4-byte address */
static const uint32_t drenr_addr_4byte = SPIM_DRENR_ADE_4BYTE
					 << SPIM_DRENR_ADE_POS;

/* DRENR value for no additional data */
static const uint32_t drenr_additional_none = SPIM_DRENR_OPDE_NONE
					      << SPIM_DRENR_OPDE_POS;

/* DRENR value for 1-byte additional data */
static const uint32_t drenr_additional_1byte = SPIM_DRENR_OPDE_1BYTE
					       << SPIM_DRENR_OPDE_POS;

/* DRENR value for 2-byte additional data */
static const uint32_t drenr_additional_2byte = SPIM_DRENR_OPDE_2BYTE
					       << SPIM_DRENR_OPDE_POS;

/* DRENR value for 3-byte additional data */
static const uint32_t drenr_additional_3byte = SPIM_DRENR_OPDE_3BYTE
					       << SPIM_DRENR_OPDE_POS;

/* DRENR value for 4-byte additional data */
static const uint32_t drenr_additional_4byte = SPIM_DRENR_OPDE_4BYTE
					       << SPIM_DRENR_OPDE_POS;
			      ...
			      xspi_op_t const *const wop)
{
	...
	uint32_t phyoffset2_msk = SPIM_PHYOFFSET2_OCTTMG;
	uint32_t phyoffset2_set = SPIM_PHYOFFSET2_SPI
				  << SPIM_PHYOFFSET2_OCTTMG_POS;
	uint32_t phycnt_msk = SPIM_PHYCNT_PHYMEM;
	...
	case 3:
		...
		dropr |= (rop->additional_value & 0xff0000) >>
			 16 << SPIM_DROPR_OPD3_POS;
		break;
	case 4:
		drenr |= drenr_additional_4byte;
		dropr |= (rop->additional_value & 0xff) << SPIM_DROPR_OPD0_POS;
		dropr |= (rop->additional_value & 0xff00) >>
			 8 << SPIM_DROPR_OPD1_POS;
		dropr |= (rop->additional_value & 0xff0000) >>
			 16 << SPIM_DROPR_OPD2_POS;
		dropr |= (rop->additional_value & 0xff000000) >>
			 24 << SPIM_DROPR_OPD3_POS;
		break;
	default:
	...
	/* Dummy cycle */
	if (rop->dummy_cycles == 1 || rop->dummy_cycles > 20) {
	...
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_SSLDR, ssldr_clear,
			   ssldr_set);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYOFFSET1, phyoffset1_msk,
			   phyoffset1_set);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYOFFSET2, phyoffset2_msk,
			   phyoffset2_set);
	mmio_clrsetbits_32(myctrl->reg_base + SPIM_PHYCNT, phycnt_msk,
			   phycnt_set | SPIM_PHYCNT_CAL);
