...
/* Defaults */
static const bool DEFAULT_PBE_ENABLE;
...
static const e_burst_read_wrap_depth_t DEFAULT_BURST_READ_WRAP_DEPTH =
	BURST_READ_WRAP_DISABLE;
static const e_output_driver_strength_t DEFAULT_OUTPUT_DRIVER_STRENGTH =
	OUTPUT_DRIVER_STRENGTH_30_OHMS;
...
static int flash_exec_op(xspidevice_ctrl_t *ctrl, xspi_op_t const *op,
			 bool is_write);
...
static int flash_read(xspidevice_ctrl_t *ctrl, void *buffer, size_t address,
		      size_t length);
static int flash_write(xspidevice_ctrl_t *ctrl, void const *buffer,
		       size_t address, size_t length);
static int flash_erase(xspidevice_ctrl_t *ctrl, size_t address, size_t length);
static enum xspidevice_write_status
flash_get_write_status(xspidevice_ctrl_t *ctrl);
...
static uint8_t
flash_read_configuration_register(qspiflash_mx25l25645g_ctrl_t *myctrl)
{
	int result;
	const xspi_instance_t *xspi = myctrl->xspi;
...
static int flash_wait_for_write_complete(qspiflash_mx25l25645g_ctrl_t *myctrl,
					 uint32_t timeout /* usec */)
{
	assert(myctrl);
	assert(timeout > 0);
...
{
	/* Set WP#SIO2 as GPIO */
	mmio_write_8(WP_IO2_PIN_PORT_MODE_ADDR,
		     mmio_read_8(WP_IO2_PIN_PORT_MODE_ADDR) & ~WP_IO2_PIN_BIT);
	/* Set WP#SIO2 to high */
	mmio_write_8(WP_IO2_PIN_PORT_ADDR,
		     mmio_read_8(WP_IO2_PIN_PORT_ADDR) | WP_IO2_PIN_BIT);
...
static void flash_hw_protect_unlock(qspiflash_mx25l25645g_ctrl_t *myctrl)
{
	...
	if (flash_prepare_unlock(myctrl) < 0)
		goto err;
	...
	if (flash_finish_unlock(myctrl) < 0)
		goto err;
...
{
	if (flash_check_manufacturer_id(myctrl) != 0)
		return -1;
	...
	if (DEFAULT_PBE_ENABLE) {
		configuration_register |= 1u
					  << 4; /* PBE (Preamble Bit Enable) */
	}
	configuration_register |= DEFAULT_OUTPUT_DRIVER_STRENGTH
				  << 0; /* ODS[1:0] */

	/* Write configuration register */
	xspi_op_t op = op_write_sr;
	uint8_t status_configuration_register[2] = { status_register,
						     configuration_register };
	op.transfer_buffer = (void *)&status_configuration_register;
	result = xspi->api->exec_op(xspi->ctrl, &op, true);
	if (result < 0)
...
static int flash_open(xspidevice_ctrl_t *ctrl, xspidevice_cfg_t const *cfg)
{
	...
	assert(cfg);
	int result;
	qspiflash_mx25l25645g_ctrl_t *myctrl =
		(qspiflash_mx25l25645g_ctrl_t *)ctrl;
	...
	if (result == 0) {
		...
		do {
			retries++;
			if (retries >= MAX_FLASH_UNLOCK_RETRIES)
				break;

			result = flash_init(myctrl);
			if (result == -EAGAIN)
				flash_hw_protect_unlock(myctrl);
		} while (result == -EAGAIN);
...
static int flash_enter_xip(xspidevice_ctrl_t *ctrl)
{
	...
	if (DEFAULT_READ_MODE == READ_MODE_QUAD) {
		result = xspi->api->configure_xip(xspi->ctrl, &op_quad_io_read,
						  NULL);
	} else {
		result = xspi->api->configure_xip(xspi->ctrl, &op_fast_read,
						  NULL);
	}
...
static int flash_get_info(xspidevice_ctrl_t *ctrl, xspidevice_info_t *info)
{
	...
	strlcpy(info->device_product, "MX25L25645G",
		sizeof(info->device_product));
	return 0;
}
static int flash_read(xspidevice_ctrl_t *ctrl, void *buffer, size_t address,
		      size_t length)
...
static enum xspidevice_write_status flash_get_write_status(xspidevice_ctrl_t *ctrl)
{
	...
	if ((result & BIT(0)) != 0)
		return WRITE_STATUS_IN_PROGRESS;
	if ((result & BIT(1)) == 0)
		return WRITE_STATUS_DISABLED;
