...
/* MMU allocation area */
uint64_t __attribute__((section("base_xlat_table")))
						mmu_level1_table[RZA_MMU_LEVEL2_TABLE_INDEX_MAX];
uint64_t __attribute__((section("xlat_table"), aligned(0x1000)))
mmu_level2_table[RZA_MMU_TABLE_ENTRY * RZA_MMU_LEVEL2_TABLE_INDEX_MAX];
...
int plat_mmu_init(const rza_mmu_pgtbl_cfg_t *config_table)
{
	...
	if (0 == ret) {
		...
		for (index = 0; index < RZA_MMU_LEVEL2_TABLE_INDEX_MAX; index++) {
			level2_table[index] =
				(uint64_t)&mmu_level2_table[RZA_MMU_TABLE_ENTRY * index];
		}

		/* Set the address of the level 2 MMU table in the level 1 MMU table */
		for (index = 0; index < RZA_MMU_LEVEL2_TABLE_INDEX_MAX; index++) {
			*((uint64_t *)&mmu_level1_table[index]) =
				(level2_table[index] | RZA_MMU_ATTR_NEXT_TBL);
		}

		rz_mmu_write_tbl(config_table);

		/* create a pattern of attributes to set in the MAIR register */
		mair = RZA_MMU_MAIR_ATTR_SET(RZA_MMU_ATTR_DEVICE,
					     RZA_MMU_ATTR_DEVICE_INDEX);
		mair |= RZA_MMU_MAIR_ATTR_SET(
			RZA_MMU_ATTR_IWBWA_OWBWA_NTR,
			RZA_MMU_ATTR_IWBWA_OWBWA_NTR_INDEX);
		mair |= RZA_MMU_MAIR_ATTR_SET(RZA_MMU_ATTR_NON_CACHEABLE,
					      RZA_MMU_ATTR_NON_CACHEABLE_INDEX);
...
void plat_mmu_enable(void)
{
	 ...
	 */
	__asm__ volatile(".equ SCTLR_C,    (1 << 2)\n"
			 ".equ SCTLR_M,    (1 << 0)\n"
			 ".equ SCTLR_WXN,  (1 << 19)\n"
...
int rz_mmu_check_tbl(const rza_mmu_pgtbl_cfg_t *config_table)
{
	...
	if (RZA_MMU_CURRENT_EL3 == current_el) {
		for (index = 0; ((0 == ret) && (RZA_MMU_ATTRIBUTE_CONFIG_END !=
						config_table[index].attribute));
		     index++) {
			...
			if (RZA_MMU_PHY_SIZE_MAX <
			    (uint64_t)(config_table[index].paddress + config_table[index].size)) {
				ret = -1;
			}

			if (RZA_MMU_VIRT_SIZE_MAX <
			    (uint64_t)(config_table[index].vaddress + config_table[index].size)) {
				ret = -1;
			}
		}
	} else {
...
void rz_mmu_write_tbl(const rza_mmu_pgtbl_cfg_t *config_table)
{
	...
	for (index = 0; RZA_MMU_ATTRIBUTE_CONFIG_END != config_table[index].attribute; index++) {
		...
		ttb_index = (config_table[index].vaddress - ttb_start_vaddr) /
			    RZA_MMU_L2_ENTRY_SIZE;
