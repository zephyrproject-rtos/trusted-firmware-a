				  ...
				  size_t length, size_t *length_read)
{
	...
	if (0 < first_offset) {
		...
		if (emmc_read_sector((uint32_t *)sector_buf, first_sector, 1,
				     emmc_dma) != EMMC_SUCCESS) {
			result = IO_FAIL;
			goto block_read_done;
		} else {
			buffer_offset = EMMC_SECTOR_SIZE - first_offset;
			buffer_offset = (length < buffer_offset) ?
						length :
						buffer_offset;

			memcpy((uint8_t *)buffer, &sector_buf[first_offset],
			       buffer_offset);
	...
	uint32_t last_offset =
		(fp->base + fp->file_pos + length) % EMMC_SECTOR_SIZE;
	if (0 < sector_count && 0 < last_offset) {
		memset(sector_buf, 0x00, EMMC_SECTOR_SIZE);
		if (emmc_read_sector((uint32_t *)sector_buf, last_sector, 1,
				     emmc_dma) != EMMC_SUCCESS) {
			result = IO_FAIL;
			goto block_read_done;
		} else {
			memcpy((uint8_t *)buffer + (length - last_offset),
			       &sector_buf[0], last_offset);
			sector_count--;
		}
	}
