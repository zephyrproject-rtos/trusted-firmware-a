...
int rz_check_fsp_header(fsp_app_header_t *header)
{
	...
	if ((FSP_BASE + RZ_APP_PAYLOAD_OFFSET) != header->dest_addr) {
		if (ret == 0) {
			if ((header->dest_addr < PLAT_SRAM_BASE) ||
			    ((header->dest_addr + header->payload_size) >
							(PLAT_SRAM_BASE + PLAT_SRAM_SIZE))) {
				if ((header->dest_addr < PLAT_SPIROM_BASE) ||
				    ((header->dest_addr + header->payload_size) >
							(PLAT_SPIROM_BASE + PLAT_SPIROM_SIZE))) {
					if ((header->dest_addr <  PLAT_DDR_BASE) ||
					    ((header->dest_addr + header->payload_size) >
...
void rza_load_fsp(void)
{
	...
	if (ret == 0) {
		if ((FSP_BASE + RZ_APP_PAYLOAD_OFFSET) != header->dest_addr) {
			rz_xspi_read((void *)header->dest_addr,
				     RZ_APP_PAYLOAD_OFFSET + FSP_FROM_XSPI_BASE,
