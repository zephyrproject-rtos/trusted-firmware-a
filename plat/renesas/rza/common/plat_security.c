...
void plat_access_control_setup(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(sys_acctl); i++) {
		uint32_t val = mmio_read_32(sys_acctl[i].reg) &
			       (~sys_acctl[i].msk);
...
uint8_t tzc400_get_num_filters(uintptr_t tzc_base)
{
	...
	return (uint8_t)((tzc400_build >> BUILD_CONFIG_NF_SHIFT) &
		       ...
		       const arm_tzc_regions_info_t *tzc_regions)
{
	...
	const arm_tzc_regions_info_t init_tzc_regions[] = { { 0 } };
	...
	tzc400_configure_region0(TZC_REGION_S_RDWR,
				 PLAT_TZC_REGION_ACCESS_NS_UNPRIV);
...
static void bl2_security_setup(void)
{
	...
	/* initialize TZC-400 */
	plat_tzc400_setup(PLAT_TZC_DDR_BASE, &ddr_tzc_regions[0]);
	plat_tzc400_setup(PLAT_TZC_SPI_BASE, NULL);
...
static void bl31_security_setup(void)
{
	...
	plat_tzc400_setup(PLAT_TZC_MSRAM_BASE, &msram_tzc_regions[0]);
	plat_tzc400_setup(PLAT_TZC_ASRAM_BASE, &asram_tzc_regions[0]);
}
#endif
