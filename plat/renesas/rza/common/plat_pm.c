...
static int rza_pwr_domain_on(u_register_t mpidr)
{
	const uint32_t rval[2][2] = {
		{ SYS_CA55_CFG_RVAL0, SYS_CA55_CFG_RVAH0 },
		{ SYS_CA55_CFG_RVAL1, SYS_CA55_CFG_RVAH1 }
	};
	const uint32_t pch[2][2] = { { CPG_CORE0_PCHCTL, CPG_CORE0_PCHMON },
...
static void rza_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
#if !DEBUG_PLAT_FPGA
	plat_gic_pcpu_init();
...
const plat_psci_ops_t rza_plat_psci_ops = {
	.pwr_domain_on = rza_pwr_domain_on,
	.pwr_domain_on_finish = rza_pwr_domain_on_finish,
};
