...
typedef struct st_xspidevice_api {
	 ...
	 */
	int (*exec_op)(xspidevice_ctrl_t *ctrl, xspi_op_t const *op,
		       bool is_write);
	 ...
	 */
	int (*read)(xspidevice_ctrl_t *ctrl, void *buffer, size_t address,
		    size_t length);
	 ...
	 */
	int (*write)(xspidevice_ctrl_t *ctrl, void const *buffer,
	 ...
	 * @param[in, out] ctrl Opened control block.
	 */
	enum xspidevice_write_status (*get_write_status)(
		xspidevice_ctrl_t *ctrl);
