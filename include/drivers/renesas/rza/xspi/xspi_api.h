...
#define XSPI_FEATURE_QUAD \
	(XSPI_FEATURE_FORM114 | XSPI_FEATURE_FORM144 | XSPI_FEATURE_FORM444)
...
#define XSPI_FLAGS_SEQUENTIAL_DDR \
	(1U << 0) /* Enable inverted order for DDR transfer */
#define XSPI_FLAGS_FORCE_STROBE \
	(1U << 1) /* Force to use of data strobe signal for timing reference */
#define XSPI_FLAGS_DATA_ACCESS \
	(1U << 2) /* Force to use data access method for exec_op */
...
/** xSPI control block.
 * Allocate an implementation specific control block to pass into the xSPI API calls.
 */
...
typedef struct st_xspi_op {
	...
	size_t transfer_size; ///< Transfer size by bytes (ignored for in-place access)
	void *transfer_buffer; ///< Transfer buffer pointer (ignored for in-place access)
	bool transfer_is_ddr; ///< Transfer phase is DDR
	uint8_t transfer_flag; ///< Additional transfer flags
	uint8_t force_idle_level_mask; ///< Mask to force each IO level during the idle state
...
typedef struct st_xspi_api {
	 ...
	 */
	int (*exec_op)(xspi_ctrl_t *const ctrl, xspi_op_t const *const op,
		       bool is_write);
	 ...
	 */
	int (*configure_xip)(xspi_ctrl_t *const ctrl,
